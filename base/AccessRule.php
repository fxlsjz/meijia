...
class AccessRule extends Component
{
    ...
    function checklogin(){
         $users = isset($_COOKIE['login']) ? json_decode($_COOKIE['login'], true) : [];
        //print_r($_COOKIE);exit;
        if (!$users || !isset($users['user_id']) || !isset($users['token'])){
            $logined = false;
        }else{
            $expires = time();
            // $cacheKey = 'token_'.$users['user_id'];
            // $token = Yii::$app->redis->get($cacheKey);
            // if ($users['token'] && $token == $users['token']) {
            //     $logined = true;
            // }
            $sql = "SELECT `expire` FROM {{%token}} where `uid` = :user_id and `token` = :token and `expire` > :expires";
            $param = ['user_id'=>$users['user_id'], 'token'=>$users['token'], 'expires'=>$expires];
            $logined = Yii::$app->db->createCommand($sql, $param)->queryScalar() ? true : false;         
        }
        return $logined;
    }
