 ...
 */
// 到家  到店 服务选择
app.controller('ServiceSingleNewCtrl', function ($rootScope, $scope, $filter, $state, $cookies, languages, getInterface, Xalert, $ionicSlideBoxDelegate, sourseChange, Authentication, $timeout, $window, SwitchEnterpriseService, userInfo, $http, SwitchShopService, serviceChange, $stateParams, $ionicPopup) {
    ...
    var loadData = function () {
        var options = {
            ...
            params: {
                ...
                service_ordertype: $scope.tabType,
                servicetype_id: $scope.servicetype_id,
...
//					current_page: $scope.page,
        ...
        $scope.doRefresh = function () {
            $scope.page = 1;
            if($scope.servicetype_id == ''){
//          	alert($scope.servicetype_id)
            }else{
//          	alert($scope.servicetype_id)
            }
            $timeout(function () {
    ...
    $scope.next = function () {
        ...
        if (selectedData && selectedData.length > 0) {
            ...
            if (type != 'SERVICE_MANY') { //套票不需要时间限制
                if (parseInt($scope.time) < $rootScope.mintime) {
